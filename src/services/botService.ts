...
export async function createBot(
  ...
  try {
    ...
    if (existingBot) {
      throw new Error(
        `A bot with the email address "${botData.emailAddress}" already exists`
      );
    }
    ...
    return {
      ...
      bot: {
        ...
        forwardEmailDisplay:
          bot.forward_email_display || "Forward your emails here",
      },
    };
  } catch (error) {
...
export async function updateBot(id: string, botData: Partial<Bot>) {
  try {
    ...
    console.log("botData.emailAddress", botData.emailAddress);
    ...
    console.log("botData check id", botData);

    // Only include fields that are actually being updated
    const updateData: Record<string, any> = {};

    if (botData.name !== undefined) updateData.name = botData.name;
    if (botData.emailAddress !== undefined)
      updateData.email_address = botData.emailAddress;
    if (botData.description !== undefined)
      updateData.description = botData.description;
    if (botData.forwardTemplateId !== undefined)
      updateData.forward_template_id = botData.forwardTemplateId;
    if (botData.forwardEmailAddress !== undefined)
      updateData.forward_email_address = botData.forwardEmailAddress;
    if (botData.includeCustomerInForward !== undefined)
      updateData.include_customer_in_forward = botData.includeCustomerInForward;
