...
export async function createKnowledgeBaseItem(itemData: Omit<KnowledgeBase, 'id' | 'status' | 'lastUpdated'>) {
  try {
    ...
    if (itemData.type === 'website') {
      try {
        ...
        throw new Error('Invalid URL format. Please include the full URL (e.g., https://example.com)');
      }
    }
    ...
    if (existingItem) {
      throw new Error('This source has already been added to the knowledge base');
    }
    ...
    if (!item) {
      throw new Error('Failed to create knowledge base item - no data returned');
    }
    ...
    return {
      success: false,
      error: error instanceof Error ? error.message : 'Failed to create knowledge base item',
    };
  }
}

export async function updateKnowledgeBaseItem(id: string, itemData: Partial<KnowledgeBase>) {
  try {
    const { data: { session }, error: sessionError } = await supabase.auth.getSession();
    ...
    return {
      success: false,
      error: error instanceof Error ? error.message : 'Failed to update knowledge base item',
    };
  }
}
...
export async function deleteKnowledgeBaseItem(id: string) {
  try {
    ...
    return {
      success: false,
      error: error instanceof Error ? error.message : 'Failed to delete knowledge base item',
    };
  }
}