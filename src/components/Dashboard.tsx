...
export default function Dashboard() {
  ...
  const handleSubmit = async (botData: {
    ...
    console.log("botData within handleSubmit", botData);
    setError(null);
    setIsLoading(true);
    try {
      if (editingBot) {
      ...
      setError(
        err instanceof Error
          ? err.message
          : "An error occurred while saving the bot"
      );
    } finally {
      setIsLoading(false);
    }
  ...
  const handleUrlAdd = async (data: WebsiteSource) => {
    try {
      ...
      setError(
        err instanceof Error
          ? err.message
          : "An error occurred while adding the URL"
      );
    }
  };

  const handleTemplateSubmit = async (
    templateData: Omit<EmailTemplate, "id" | "lastModified" | "createdBy">
  ) => {
    try {
      if (editingTemplate) {
  ...
  const handleAddFineTuningQuestion = async (
  ...
  const handleEditFineTuningQuestion = async (
    id: string,
  ...
  const handleExportConversations = (botId: string) => {
    const exportConversations =
      botId === "all"
        ? conversations
        : conversations.filter((conv) => conv.botId === botId);
    const filename =
      botId === "all"
        ? "all-conversations.csv"
        : `${bots
            .find((b) => b.id === botId)
            ?.name.toLowerCase()
            .replace(/\s+/g, "-")}-conversations.csv`;
  ...
  const currentMonth = format(new Date(), "yyyy-MM"); // Example: "2025-03"
  const previousMonth = format(subMonths(new Date(), 1), "yyyy-MM"); // Example: "2025-02"

  // Filter conversations by month
  const currentMonthConversations = conversations.filter((c) =>
    c.startedAt?.startsWith(currentMonth)
  );
  const prevMonthConversations = conversations.filter((c) =>
    c.startedAt?.startsWith(previousMonth)
  );

  // Filter bots by month
  const currentMonthBots = bots.filter((b) =>
    b.created_at?.startsWith(currentMonth)
  );
  const prevMonthBots = bots.filter((b) =>
    b.created_at?.startsWith(previousMonth)
  );

  // Calculate metrics
  const totalConversations = currentMonthConversations.length;
  const prevTotalConversations = prevMonthConversations.length;
  const totalMessages = currentMonthConversations.reduce(
    (acc, conv) => acc + conv.totalMessages,
    0
  );
  const prevTotalMessages = prevMonthConversations.reduce(
    (acc, conv) => acc + conv.totalMessages,
    0
  );
  const activeBots = currentMonthBots.filter(
    (b) => b.status === "active"
  ).length;
                ...
                {bots.map((bot) => (
                  ...
                  >
                    ...
                    <div className="space-y-3 text-sm">
                      ...
                      <p className="text-gray-600">
                        Last Active: {formatDate(bot.last_active)}
                      </p>
                      {(bot.forward_template_id ||
                        bot.forward_email_address) && (
                          ...
                          <div className="space-y-2">
                            {bot.forward_email_address && (
                              <div>
                                ...
                                <p className="text-xs text-gray-500">
                                  {bot.include_customer_in_forward
                                    ? "Customer will be CC'd"
                            ...
                            {bot.forward_template_id && (
                              <div>
                                ...
                                <p className="text-sm text-gray-700">
                                  {templates.find(
                                    (t) => t.id === bot.forward_template_id
              ...
              <div className="bg-white rounded-lg shadow overflow-hidden">
                ...
                {knowledgeBase.length === 0 && (
                  <div className="text-center py-12">
                    ...
                    <p className="mt-1 text-sm text-gray-500">
                      Upload documents or add website URLs to train your bots
                    </p>
                    <div className="mt-6 flex justify-center gap-3">
                      <button
                        onClick={() => setIsUrlModalOpen(true)}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                ...
                <AnalyticsCard
                  ...
                  value={bots
                    ...
                    .length.toString()}
                  icon={BotIcon}
                  change={activeBots - prevActiveBots}
                />
                <AnalyticsCard
                ...
                <AnalyticsCard
                  ...
                  value={conversations
                    .reduce((acc, conv) => acc + conv.totalMessages, 0)
                    .toString()}
                  icon={MessageCircle}
                  change={getPercentageChange(totalMessages, prevTotalMessages)}
                />
              </div>
            </div>
          )}
